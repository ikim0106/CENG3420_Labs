...
void handle_srli(unsigned int cur_inst)
{
     ...
     */
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    unsigned int imm12 = sext(MASK31_20(cur_inst), 12);
    NEXT_LATCHES.REGS[rd] = CURRENT_LATCHES.REGS[rs1] >> imm12;
    // printf("SRLI INSTRUCTION: %d\n", imm12);
}
...
void handle_sub(unsigned int cur_inst)
{
     ...
     * Lab2-2 assignment
     */
    signed int rd = MASK11_7(cur_inst),
               rs1 = MASK19_15(cur_inst),
               rs2 = MASK24_20(cur_inst);
    NEXT_LATCHES.REGS[rd] = CURRENT_LATCHES.REGS[rs1] - CURRENT_LATCHES.REGS[rs2];
}
...
void handle_lb(unsigned int cur_inst)
{
    ...
    // NEXT_LATCHES.REGS[rd] = MEMORY[sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1] + 4];
}
...
void handle_sb(unsigned int cur_inst)
{
     ...
     */
    unsigned int rs1 = MASK19_15(cur_inst), rs2 = MASK24_20(cur_inst);
    unsigned int imm12 = (MASK31_25(cur_inst) << 5) + (MASK11_7(cur_inst));
...
void handle_sh(unsigned int cur_inst)
{
    ...
    unsigned int imm12 = (MASK31_25(cur_inst) << 5) + (MASK11_7(cur_inst));
    MEMORY[imm12 + CURRENT_LATCHES.REGS[rs1]] = NEXT_LATCHES.REGS[rs2];
    MEMORY[imm12 + CURRENT_LATCHES.REGS[rs1] + 1] = NEXT_LATCHES.REGS[rs2] >> 8;
...
void handle_sw(unsigned int cur_inst)
{
    ...
    MEMORY[imm12 + CURRENT_LATCHES.REGS[rs1]] = NEXT_LATCHES.REGS[rs2];
    MEMORY[imm12 + CURRENT_LATCHES.REGS[rs1] + 1] = NEXT_LATCHES.REGS[rs2] >> 8;
    MEMORY[imm12 + CURRENT_LATCHES.REGS[rs1] + 2] = NEXT_LATCHES.REGS[rs2] >> 16;
    MEMORY[imm12 + CURRENT_LATCHES.REGS[rs1] + 3] = NEXT_LATCHES.REGS[rs2] >> 24;
...
void handle_instruction()
{
    ...
    case (0x03):
        ...
        case 2:
            handle_lw(cur_inst);
            break;
        // don't handle 4 and 5
        default:
            error("bruh");
        }
        break;
    case (0x0C << 2) + 0x03:
        // handle register arithmetric instructions
        switch (funct3)
        {
        case 0:
            if (MASK31_25(cur_inst) == 0)
